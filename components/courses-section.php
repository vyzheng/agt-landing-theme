<!-- Courses Section -->
<div class="courses-section" id="pricing">
    <div class="courses-container">
        <!-- Section Header -->
        <div class="section-header">
            <h2 class="section-title">Choose Your Path</h2>
        </div>
        
        <!-- Toggle Tabs -->
        <div class="course-tabs">
            <button class="tab-button active" onclick="toggleTab('self-paced')">Self-paced courses</button>
            <button class="tab-button" onclick="toggleTab('personal-guidance')">1-1 with Dr. Vivian</button>
        </div>
        
        <!-- Self-paced courses tab content -->
        <div id="self-paced" class="tab-content active">
            <!--<h3 class="guidance-title">Admissions Game Theory</h3>-->
            <div class="courses-grid">
                <div class="course-card">
                    <span class="course-icon">📊</span>
                    <h3 class="course-title">Strategizer</h3>
                    <p class="course-tagline">Master the admissions game with data-driven insights</p>
                    <div class="course-price">$399</div>
                    <ul class="course-features">
                        <li>Real application data analysis</li>
                        <li>ROI framework for activities</li>
                        <li>Strategic roadmap creation</li>
                    </ul>
                    <a href="https://buy.stripe.com/8x28wO0socU53sBgHhaAw02" class="course-cta">Start Now</a>
                </div>
                
                <div class="course-card flagship">
                    <div class="popular-badge">Most Popular</div>
                    <span class="course-icon">🏆</span>
                    <h3 class="course-title">Flagship</h3>
                    <p class="course-tagline">The complete system for elite admissions success</p>
                    <div class="course-price">$699</div>
                    <ul class="course-features">
                        <li>Everything in both courses</li>
                        <li>150K+ application insights</li>
                        <li>Complete admissions mastery</li>
                    </ul>
                    <a href="https://buy.stripe.com/8x214mgrmdY98MVcr1aAw00" class="course-cta">Start Now</a>
                </div>
                
                <div class="course-card">
                    <span class="course-icon">✍️</span>
                    <h3 class="course-title">Storyteller</h3>
                    <p class="course-tagline">Craft compelling narratives that captivate admissions</p>
                    <div class="course-price">$399</div>
                    <ul class="course-features">
                        <li>Essay prompt decoding</li>
                        <li>Real example analysis</li>
                        <li>Story structure mastery</li>
                    </ul>
                    <a href="https://buy.stripe.com/00w14m3EA6vHd3b76HaAw03" class="course-cta">Start Now</a>
                </div>
            </div>
        </div>

        <!-- Personal guidance tab content -->
        <div id="personal-guidance" class="tab-content">
            <!--<h3 class="guidance-title">Personal Guidance with Dr. Vivian</h3>-->
            
            <div class="session-grid">
                <div class="session-card">
                    <span class="session-icon">💡</span>
                    <h4 class="session-title">Single Session</h4>
                    <p class="session-tagline">Quick expert guidance when you need it</p>
                    <div class="session-price">$350</div>
                    <ul class="session-features">
                        <li>45-minute focused consultation</li>
                        <li>Strategy brainstorming & feedback</li>
                        <li>Actionable next steps</li>
                    </ul>
                    <a href="https://calendly.com/admissionsgametheory" target="_blank" class="session-cta">Book Now</a>
                </div>

                <div class="session-card featured">
                    <div class="popular-badge">Most Popular</div>
                    <span class="session-icon">🎯</span>
                    <h4 class="session-title">Strategy Bundle</h4>
                    <p class="session-tagline">Complete positioning transformation</p>
                    <div class="session-price">$999</div>
                    <ul class="session-features">
                        <li>3 sessions over 2-3 weeks</li>
                        <li>Spike design & roadmap creation</li>
                        <li>Rating optimization strategy</li>
                    </ul>
                    <a href="https://calendly.com/admissionsgametheory" target="_blank" class="session-cta">Book Now</a>
                </div>

                <div class="session-card featured">
                    <div class="popular-badge">Most Popular</div>
                    <span class="session-icon">✍️</span>
                    <h4 class="session-title">Essay Bundle</h4>
                    <p class="session-tagline">From blank page to compelling narrative</p>
                    <div class="session-price">$999</div>
                    <ul class="session-features">
                        <li>3 sessions over 2-3 weeks</li>
                        <li>Story development & structure</li>
                        <li>Draft refinement to final polish</li>
                    </ul>
                    <a href="https://calendly.com/admissionsgametheory" target="_blank" class="session-cta">Book Now</a>
                </div>
            </div>
        </div>
    </div>
</div>